...
class PartsInOrdersTab(QWidget):
    def __init__(self, db):
        super().__init__()
        ui_path = os.path.join(os.path.dirname(__file__), '..', 'ui', 'tab_parts_in_orders.ui')
        loadUi(ui_path, self)

        self.qt_db = db

        self.model = QSqlTableModel(self, self.qt_db)
        self.model.setTable('parts_in_orders')  # Название таблицы в БД
