...
class Services_in_orders_Tab(QWidget):
    def __init__(self, db):
        super().__init__()
        ui_path = os.path.join(os.path.dirname(__file__), '..', 'ui', 'tab_services_in_orders.ui')
        loadUi(ui_path, self)

        self.qt_db = db

        self.model = QSqlTableModel(self, self.qt_db)
        self.model.setTable('services_in_orders')  # Название таблицы в БД
...
if __name__ == "__main__":
    ...
    window = Services_in_orders_Tab(db)
    window.show()
    sys.exit(app.exec())
