...
class PaymentTypesTab(QWidget):
    def __init__(self, db):
        super().__init__()
        ui_path = os.path.join(os.path.dirname(__file__), '..', 'ui', 'tab_payment_types.ui')
        loadUi(ui_path, self)

        self.qt_db = db

        self.model = QSqlTableModel(self, self.qt_db)
        self.model.setTable('payment_types')  # Название таблицы в БД
